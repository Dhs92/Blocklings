...
public class BlocklingBonemealSaplingsGoal extends Goal
{
    ...
    public BlocklingBonemealSaplingsGoal(BlocklingEntity blockling)
    {
    ...
    {
        if (!blockling.aiManager.isActive(AIManager.BONEMEAL_SAPLINGS_ID)) return false;

        if (InventoryUtil.find(blockling, Items.BONE_MEAL).index == -1) return false;

        if (!findSapling()) return false;

        return true;
    }

    private boolean findSapling()
    {
        ...
        for (int y = startY; y < endY; y++)
        {
            for (int x = startX; x < endX; x++)
            {
                for (int z = startZ; z < endZ; z++)
                {
                    ...
                    BlocklingWhitelist whitelist = blockling.aiManager.getWhitelist(AIManager.CHOP_NEARBY_ID, AIManager.BONEMEAL_SAPLINGS_WHITELIST_ID);
                    if ((whitelist == null && BlockUtil.isSapling(testBlock)) || (whitelist != null && whitelist.isInWhitelist(testBlock)))
                    {
    ...
    private boolean isValidTarget()
    {
        ...
        BlocklingWhitelist whitelist = blockling.aiManager.getWhitelist(AIManager.CHOP_NEARBY_ID, AIManager.BONEMEAL_SAPLINGS_WHITELIST_ID);
        if ((whitelist == null && !BlockUtil.isSapling(block)) || (whitelist != null && whitelist.isInBlacklist(block)))
        {
            return false;
        }
